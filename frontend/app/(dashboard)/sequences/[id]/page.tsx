...
export default function SequenceDetailPage() {
  ...
  const { data: sequence, isLoading } = useQuery({
    ...
    refetchInterval: (query) => {
      const data = query.state.data as Sequence | undefined
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => sequencesApi.stats(Number(id)).then(r => r.data),
    enabled: !!id,
  ...
  const { data: enrollments } = useQuery({
    ...
    queryFn: () => sequencesApi.enrollments(Number(id), { page_size: 10 }).then(r => r.data),
    enabled: !!id,
  })

  const activateMut = useMutation({
    mutationFn: () => sequencesApi.activate(Number(id)),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sequence', id] }); toast.success('Sequence activated') },
    onError: (err: any) => toast.error(err.response?.data?.detail || 'Failed to activate'),
  })

  const pauseMut = useMutation({
    mutationFn: () => sequencesApi.pause(Number(id)),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sequence', id] }); toast.success('Sequence paused') },
  })

  const resumeMut = useMutation({
    mutationFn: () => sequencesApi.resume(Number(id)),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sequence', id] }); toast.success('Sequence resumed') },
  })

  const deleteMut = useMutation({
    mutationFn: () => sequencesApi.delete(Number(id)),
    onSuccess: () => { toast.success('Sequence deleted'); router.push('/sequences') },
  })
    ...
    <div className="p-6 space-y-6">
      <div className="rounded-2xl border bg-card p-5 flex items-center justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3 min-w-0">
          <Button variant="ghost" size="icon-sm" onClick={() => router.push('/sequences')} className="shrink-0">
            <ArrowLeft size={16} />
          </Button>
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-xl font-bold truncate">{sequence.name}</h1>
              <StatusBadge status={sequence.status} />
            </div>
            <p className="text-sm text-muted-foreground mt-0.5">{sequence.steps.length} step(s)</p>
          </div>
        </div>

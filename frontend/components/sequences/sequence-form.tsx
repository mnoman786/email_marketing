...
import { sequencesApi, listsApi, smtpApi } from '@/lib/api'
...
function toLocalSteps(steps?: SequenceStep[]): LocalStep[] {
  return (steps || []).map(s => ({
    id: s.id,
    order: s.order,
    subject: s.subject,
...
export function SequenceForm({ sequence }: Props) {
  const router = useRouter()
  const [tab, setTab] = useState<'details' | 'steps' | 'smtp'>('details')
  const [steps, setSteps] = useState<LocalStep[]>(
    toLocalSteps(sequence?.steps).length ? toLocalSteps(sequence?.steps) : [
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: {
      ...
      contact_list_ids: sequence?.contact_list_ids || [],
      track_opens: sequence?.track_opens ?? true,
  ...
  async function persistSteps(sequenceId: number) {
    for (const stepId of deletedStepIds) {
      await sequencesApi.deleteStep(sequenceId, stepId)
    }
    for (const step of steps) {
      ...
      if (step.id) {
        await sequencesApi.updateStep(sequenceId, step.id, payload)
      } else {
        await sequencesApi.createStep(sequenceId, payload)
      }
    }
  }
  ...
  const saveMut = useMutation({
    mutationFn: async (data: FormData) => {
      const res = sequence
        ? await sequencesApi.update(sequence.id, data)
        : await sequencesApi.create(data)
      await persistSteps(res.data.id)
      if (useCustomSMTP && smtpRoutes.length > 0) {
        await sequencesApi.updateSmtpRoutes(res.data.id, smtpRoutes)
      }
      return res.data
    },
    onSuccess: (data) => {
  ...
  const activateMut = useMutation({
    mutationFn: async (data: FormData) => {
      ...
      await sequencesApi.activate(res.data.id)
      return res.data
    },
    onSuccess: (data) => {
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 py-4 border-b bg-card">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-semibold">{sequence ? 'Edit Sequence' : 'New Sequence'}</h1>
            <p className="text-xs text-muted-foreground">Build a multi-step drip sequence</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {tab === 'details' && (
          <div className="max-w-2xl space-y-5">
            <Card>
              <CardHeader><CardTitle className="text-sm">Sequence Info</CardTitle></CardHeader>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Halt the sequence as soon as a reply is detected (requires IMAP reply detection enabled on an SMTP account)
                    </p>
                  </div>
              ...
              <Card key={step.id ?? `new-${index}`}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <div>
                      <Label>Stop sequence if opened</Label>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <Label>Stop sequence if clicked</Label>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Override default SMTP weights for this sequence</p>
                  </div>
                ...
                {!useCustomSMTP && (
                  <div className="rounded-lg bg-muted/50 p-4 text-sm text-muted-foreground">
                    ...
                    <p className="mt-1">Enable custom routing above to set per-sequence weights.</p>
                  </div>
                )}
              </CardContent>
            </Card>
